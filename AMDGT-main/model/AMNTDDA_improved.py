...
class AMNTDDA_improved(nn.Module):
    def __init__(self, args, topo_drug_dim=12, topo_disease_dim=12, topo_gate_init=-1.0):
        ...
        # ==== IMPROVED: Gated Fusion & Topo Injection ====
        self.drug_gated_fusion = GatedFusion(args.gt_out_dim)
        self.disease_gated_fusion = GatedFusion(args.gt_out_dim)
        
        self.drug_topo_inject = GatedTopoInjection(topo_drug_dim, args.gt_out_dim, init_gate=topo_gate_init)
        self.disease_topo_inject = GatedTopoInjection(topo_disease_dim, args.gt_out_dim, init_gate=topo_gate_init)

        # ==== IMPROVED: Residual MLP Head ====
        # We increase the robustness of the classification head
        # NEW V12: Triple-Path Input (Fused + Sim + HGT) 
        # Dual-Path Head (Mul + Cat) input dimension = gt_out_dim * 9
        self.mlp_in = nn.Linear(args.gt_out_dim * 9, 1024)
    ...
    def forward(self, drdr_graph, didi_graph, drdipr_graph,
        ...
        # 4. NEW: Topo Feature Injection (Critical for sparse datasets like F)
        ...
        # 6. Classification with Dual-Path Residual MLP
        dr_s = dr[sample[:, 0]]
        di_s = di[sample[:, 1]]
        
        # Path 1: Element-wise multiplication (Correlation)
        mul_feat = torch.mul(dr_s, di_s)
        # Path 2: Concatenation (Feature preservation)
        cat_feat = torch.cat([dr_s, di_s], dim=-1)
        # Combine both paths
        drdi_embedding = torch.cat([mul_feat, cat_feat], dim=-1)
