...
class MultiHeadAttentionLayer(nn.Module):
    def __init__(self, in_dim, out_dim, num_heads, use_bias):
        ...
        # Learnable scaling for attention scores
        self.scale = nn.Parameter(torch.FloatTensor([np.sqrt(self.out_dim)]))
    ...
    def propagate_attention(self, g):
        ...
        # Use learnable scale instead of constant
        def scaled_exp_learnable(field, scale_param):
            def func(edges):
                return {field: torch.exp((edges.data[field] / scale_param).clamp(-5, 5))}
            return func
            
        g.apply_edges(scaled_exp_learnable('score', self.scale))
