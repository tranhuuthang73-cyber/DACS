...
Usage:
  python train_DDA_improved.py --dataset F-dataset --version v1
...
from model.AMNTDDA_improved_F import AMNTDDA_improved_F
...
def _select_threshold(y_true, y_prob, strategy="mcc", grid_size=401):
    ...
    strategy: mcc | f1 | min_discrete | aupr | balanced_recall
    """
    ...
    for thr in thresholds:
        ...
        metrics = _compute_discrete_metrics(y_true, y_pred)
        if strategy == "mcc":
            obj = metrics["Mcc"]
        elif strategy == "f1":
            obj = metrics["F1-score"]
        elif strategy == "aupr":
            # AUPR-oriented: balance recall with reasonable precision
            # Obj = recall + beta * precision, beta=0.5 to favor recall
            obj = metrics["Recall"] + 0.5 * metrics["Precision"]
        elif strategy == "balanced_recall":
            # Balance recall and precision equally
            obj = (metrics["Recall"] + metrics["Precision"]) / 2
        elif strategy == "recall_boost":
            # Heavily prioritize Recall (70%) over Precision (30%)
            # Use this when Precision has large headroom but Recall is lagging
            obj = 0.7 * metrics["Recall"] + 0.3 * metrics["Precision"]
        elif strategy == "min_discrete":
            obj = min(metrics["Accuracy"], metrics["Precision"], metrics["Recall"], metrics["F1-score"], metrics["Mcc"])
        else:
            raise ValueError(f"Unknown threshold strategy: {strategy}")

        # Prefer more conservative thresholds only when tied (stability)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pos_weight', type=float, default=1.0, help='Positive class weight to boost Recall (F-dataset specific)')
    ...
    if args.loss_type == "focal":
        ...
        # F-dataset specific: Weighted Cross Entropy to boost recall
        ...
        for epoch in range(args.epochs):
            ...
            with torch.no_grad():
                ...
                if args.no_val:
                    ...
                    # recall_boost: prioritize Recall 70% / Precision 30%
                    # Precision has +0.067 headroom, Recall needs every bit of help
                    thr, _ = _select_threshold(_to_cpu(Y_test), eval_prob, strategy="recall_boost", grid_size=args.threshold_grid)
