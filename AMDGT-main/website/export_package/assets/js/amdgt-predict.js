...
// --- Main AI & Visualization Logic ---
    ...
    function renderGNN3DGraph(predictions, type, queryIdx) {
        ...
        // Build GNN-style 3D network visualization using D3.js + CSS 3D transforms
        const width = container.offsetWidth || 900;
        const height = 500;

        // Prepare nodes and edges from predictions
        const nodeSet = new Map(); // idx -> {name, type, score, layer}
        const links = [];

        // Query node (center)
        const queryName = type === 'drug'
            ? (document.getElementById('drug-search')?.value || `Drug #${queryIdx}`)
            : (document.getElementById('disease-search')?.value || `Disease #${queryIdx}`);
        nodeSet.set('query', {
            name: queryName,
            type: type,
            layer: 0,
        ...
        predictions.forEach((pred, i) => {
            const name = pred.name || `Node #${pred.disease_idx || pred.drug_idx}`;
            const predType = type === 'drug' ? 'disease' : 'drug';
            const idx = pred.disease_idx || pred.drug_idx || i;
            let rawScore = pred.score || pred.probability || 0.5;
            let normScore = rawScore > 1 ? rawScore / 100 : rawScore;
            ...
            nodeSet.set(predType + '_' + idx, {
                ...
                score: normScore,
                isQuery: false,
                isKnown: pred.is_known || false
            });
            // Link from query to prediction
            links.push({
                source: 'query',
                target: predType + '_' + idx,
                weight: normScore
            });
        });
        ...
        if (typeof ForceGraph3D !== 'undefined' && canvasEl) {
            const Graph = ForceGraph3D()(canvasEl)
                ...
                .backgroundColor('rgba(0,0,0,0)') // Transparent to show CSS rings
                .nodeId('id')
                .nodeVal(n => n.type === 'protein' ? 12 : 18) // Adjust sizes to look like the image
                .nodeColor(n => {
                ...
                .nodeLabel(n => `
                    <div style="background:rgba(20,25,40,0.95);padding:10px 14px;border-radius:12px;border:1px solid rgba(255,255,255,0.08);box-shadow:0 8px 24px rgba(0,0,0,0.5);font-family:sans-serif;min-width:120px;">
                        <div style="color:#f8fafc;font-weight:bold;font-size:15px;margin-bottom:6px;">${n.name}</div>
                        <div style="color:${n.type === 'drug' ? '#3b82f6' : (n.type === 'disease' ? '#ef4444' : '#f59e0b')};font-size:12px;font-family:monospace;font-weight:bold;">${n.id || (n.type.toUpperCase() + ' NODE')}</div>
                    </div>
                `)
                .linkColor(() => 'rgba(45,212,191,0.4)') // Using a teal-ish link color like in the image
                .linkWidth(1.5)
            ...
            // Adjust graph forces to make it spread out beautifully
        ...
        container.innerHTML = '<div style="color:#f87171; text-align:center; padding: 2rem;">Lỗi tải đồ thị 3D. Vui lòng thử lại.</div>';
    });
    }
    ...
    function predictDrug() {
        ...
        if (!idx && idx !== '0') {
            if (textQuery.length > 0) {
                ...
                const normQuery = textQuery.toLowerCase().trim();
                const normCompact = normQuery.replace(/\s+/g, '');
                ...
                const findBestMatch = (items) => {
                    ...
                    const exact = items.find(i => {
                        ...
                        const did = (i.drug_id || '').toLowerCase().trim();
                        const didCompact = did.replace(/\s+/g, '');
                        return name === normQuery || did === normQuery || didCompact === normCompact;
                    });
                    if (exact) return exact;

                    const startsWith = items.find(i => {
                        const name = (i.name || '').toLowerCase();
                        const did = (i.drug_id || '').toLowerCase();
                        return name.startsWith(normQuery) || did.startsWith(normQuery);
                    });
                    return startsWith || items[0];
                };

                const resolveFromDataset = (ds) =>
                    fetch(`api/search.php?type=drug&q=${encodeURIComponent(textQuery)}&dataset=${ds}`)
                        .then(r => r.json())
                        .then(items => ({ items, ds }));

                resolveFromDataset(globalDs)
                    .then(({ items }) => {
                        if (gen !== currentPredictionGeneration) return null;
                        const match = findBestMatch(items);
                        if (match || globalDs === 'ALL') return match;
                        return resolveFromDataset('ALL').then(({ items: allItems }) => {
                            if (gen !== currentPredictionGeneration) return null;
                            return findBestMatch(allItems);
                        });
                    })
                    .then((match) => {
                        ...
                        if (match) {
                            ...
                            document.getElementById('drug-dataset').value = match.dataset;
                            predictDrug();
                        } else {
                            alert('Không tìm thấy thuốc này trong hệ thống. Vui lòng thử dataset ALL hoặc chọn từ gợi ý.');
                ...
                return;
            } else { alert('Hãy nhập tên thuốc hoặc ID'); if (btn) resetButton(btn, 'PHÂN TÍCH THUỐC', 'fa-brain'); return; }
        }

        showLoading();

        // Gọi AI Server qua predict.php trước
        fetch('api/predict.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ type: 'drug_to_disease', drug_idx: parseInt(idx), top_k: parseInt(topk), dataset: dataset })
        })
            .then(r => r.json())
            .then(data => {
                if (data.error) throw new Error(data.error);
                handleDrugResults(data);
            })
            // Nếu AI Server lỗi, dùng proxy.php để tính toán offline
            .catch(() => {
                return fetch('api/proxy.php?action=predict_drug', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ drug_idx: parseInt(idx), top_k: parseInt(topk), dataset: dataset })
                })
                .then(r => r.json())
                .then(data => {
                    if (data.error) throw new Error(data.error);
            ...
            .catch(err => {
                ...
                alert('Lỗi dự đoán: ' + err.message + '\n\nVui lòng kiểm tra:\n1. Database đã được setup chưa (setup_db.php)\n2. AI Server đã chạy chưa (python ai_server.py)');
                hideLoading();
            });
    }
    ...
    function handleDrugResults(data) {
        ...
        // Auto render 3D GNN graph after prediction
        renderGNN3DGraph(data.predictions, 'drug', idx);

        // Auto switch to 3D tab
        const tab3d = document.querySelector('.viz-tab-btn[onclick*="\'3d\'"]');
        if (tab3d) {
            switchVizTab(tab3d, '3d');
        } else {
            // Fallback: click by text
            document.querySelectorAll('.viz-tab-btn').forEach(t => {
                if (t.textContent.includes('Đồ Thị 3D')) switchVizTab(t, '3d');
            });
        }
    ...
    function predictDisease() {
        ...
        const dataset = document.getElementById('disease-dataset').value || 'C-dataset';
        ...
        fetch('api/predict.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ type: 'disease_to_drug', disease_idx: parseInt(idx), top_k: parseInt(topk), dataset: dataset })
        })
            .then(r => {
            ...
            .then(data => {
                if (data.error) { alert(data.error); hideLoading(); if (btn) resetButton(btn, 'PHÂN TÍCH BỆNH', 'fa-brain'); return; }
                if (!data.predictions || data.predictions.length === 0) {
            ...
            .catch(err => {
                ...
                alert('Lỗi kết nối: ' + err.message + '\n\nCó thể AI Server đang không hoạt động.\nVui lòng kiểm tra ai_server.py đã được chạy chưa!');
                hideLoading();
                if (btn) resetButton(btn, 'PHÂN TÍCH BỆNH', 'fa-brain');
            });
    }
    ...
    function predictProtein() {
        ...
        fetch('api/predict.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ type: 'protein_to_any', protein_idx: parseInt(idx), top_k: parseInt(topk), dataset: document.getElementById('protein-dataset').value || 'C-dataset' })
        })
            .then(r => r.json())
            .then(data => {
                ...
                document.getElementById('results-grid').innerHTML = `
                    <div class="result-card" style="grid-column: 1/-1; text-align:center; padding: 3rem;">
                        <i class="fas fa-dna" style="font-size: 3rem; color: #f472b6; margin-bottom: 1rem;"></i>
                        <h3>Tính năng Protein Analysis</h3>
                        <p style="color: var(--text-muted);">Phân tích protein qua drug-protein-disease pathway đang được phát triển.</p>
                        <p style="color: var(--text-muted); font-size: 0.9rem;">Protein đã chọn: ${document.getElementById('protein-search').value}</p>
                        <p style="color: var(--text-muted); font-size: 0.85rem;">Tính năng đầy đủ sẽ có trong phiên bản tiếp theo.</p>
                    </div>
                `;
                document.getElementById('stats-badges').innerHTML = `
                    <div class="stat-badge" style="border: 1px solid rgba(236, 72, 153, 0.2);">
                        <div class="badge-icon" style="background:rgba(236, 72, 153, 0.1);color:#f472b6;"><i class="fas fa-dna"></i></div>
                        <div><div class="badge-value" style="color:#f472b6;">${topk}</div><div class="badge-label">Top K</div></div>
                    </div>
                    <div class="stat-badge" style="border: 1px solid rgba(245, 158, 11, 0.2);">
                        <div class="badge-icon" style="background:rgba(245, 158, 11, 0.1);color:#fbbf24;"><i class="fas fa-tools"></i></div>
                        <div><div class="badge-value" style="color:#fbbf24;">Developing</div><div class="badge-label">Status</div></div>
                    </div>
                `;
                hideLoading();
                if (btn) resetButton(btn, 'PHÂN TÍCH PROTEIN', 'fa-brain');
        ...
        document.getElementById('results-grid').innerHTML = predictions.map((p, i) => {
            ...
            <div class="result-card">
                ...
                <div class="btn-section">
                    ...
                    <button class="mini-action-btn" onclick="openIntelligenceHub(${drugIdx}, ${diseaseIdx}, '${safeName}', ${score}, ${p.is_known ? 1 : 0}, 'clinical')">
                        <i class="fas fa-info-circle"></i> CT
                    </button>
                </div>
            </div>
            `;
    ...
    window.openIntelligenceHub = function(drugIdx, diseaseIdx, targetName, score, isKnown, defaultTab = 'xai') {
        const drugName = document.getElementById('drug-search').value || 'Selected Drug';
        const diseaseName = targetName;
        const dataset = document.getElementById('global-dataset')?.value || 'C-dataset';
        ...
        function renderHub(drugInfo, similarData) {
            ...
            const hubContent = `<style>
                .htab{background:transparent;border:none;border-bottom:3px solid transparent;color:#475569;padding:14px 4px;font-size:0.72rem;font-weight:800;cursor:pointer;transition:.3s;white-space:nowrap;letter-spacing:.5px}
                .htab.active{color:#00ffcc;border-bottom-color:#00ffcc}
                .htab:hover{color:#e2e8f0}
                .hcard{background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:16px;padding:1.4rem}
                .hrow{display:flex;justify-content:space-between;align-items:center;font-size:0.78rem;border-bottom:1px solid rgba(255,255,255,0.04);padding:9px 0}
                .hrow:last-child{border:none}
                .hrow .lbl{color:#64748b}.hrow .val{color:#e2e8f0;font-weight:600}
                .hbar-bg{height:6px;background:rgba(255,255,255,0.06);border-radius:99px;overflow:hidden;margin-top:4px}
                .hbar-fill{height:100%;border-radius:99px;transition:width 1s ease}
                .htag{display:inline-block;padding:2px 8px;border-radius:6px;font-size:0.62rem;font-weight:800;margin-right:4px}
                .tooltip-vi{font-size:0.62rem;color:#475569;font-style:italic;margin-left:4px}
            </style>
            <div style="background:var(--bg-primary);color:#fff;font-family:'Inter',sans-serif;border-radius:0 0 24px 24px;">
                <!-- HEADER -->
                <div style="padding:1.8rem 2rem;border-bottom:1px solid rgba(0,255,204,0.1);display:grid;grid-template-columns:1fr auto;gap:1rem;align-items:center;background:linear-gradient(135deg,rgba(99,102,241,0.05),rgba(0,255,204,0.03))">
                    <div>
                        <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
                            <span style="background:#00ffcc;color:#000;padding:2px 10px;border-radius:20px;font-size:0.6rem;font-weight:900;letter-spacing:1px">● ĐANG HOẠT ĐỘNG</span>
                            <span style="font-size:0.65rem;color:#475569;font-family:'JetBrains Mono',monospace">${drugInfo.drug_id||'N/A'}</span>
                        </div>
                        <h2 style="font-size:1.9rem;font-weight:900;margin:0 0 4px;letter-spacing:-1px">${drugInfo.name||drugName}</h2>
                        <div style="font-size:0.8rem;color:#64748b">Phân tích liên kết với: <span style="color:#a5b4fc;font-weight:600">${diseaseName}</span></div>
                    </div>
                    <div style="text-align:center;background:${scoreBg};border:1px solid ${scoreColor}33;border-radius:20px;padding:1rem 1.5rem">
                        <div style="font-size:2.8rem;font-weight:900;color:${scoreColor};line-height:1">${score.toFixed(1)}%</div>
                        <div style="font-size:0.6rem;color:#475569;font-weight:800;letter-spacing:1px;margin-top:2px">ĐIỂM DỰ ĐOÁN AI</div>
                        <span style="background:${scoreColor}22;color:${scoreColor};padding:2px 10px;border-radius:20px;font-size:0.65rem;font-weight:800;margin-top:6px;display:inline-block">${scoreLabel}</span>
                    </div>
                </div>
                <!-- BODY -->
                <div id="hub-content-body" style="padding:2rem;min-height:420px;max-height:65vh;overflow-y:auto;overflow-x:hidden;"></div>
            </div>`;
    ...
    function initHubLogic(dInfo, sData, dName, sVal, isKnown, pDrugIdx, pDiseaseIdx) {
        ...
        body.innerHTML = `
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;margin-bottom:1.5rem;">
                <div>
                    <div class="hcard" style="margin-bottom:1rem">
                        <div style="font-size:0.65rem;color:#00ffcc;font-weight:800;letter-spacing:1px;margin-bottom:1rem">📊 HỒ SƠ ĐẶC TRƯNG AI</div>
                        ${barsHtml}
                    </div>
                    <div class="hcard" style="background:rgba(99,102,241,0.05);border-color:rgba(99,102,241,0.2)">
                        <div style="font-size:0.65rem;color:#818cf8;font-weight:800;letter-spacing:1px;margin-bottom:.8rem">🔍 NHẬN XÉT CỦA MÔ HÌNH GNN</div>
                        <p style="font-size:0.82rem;color:#94a3b8;line-height:1.7;margin:0">Mô hình <b style="color:#a5b4fc">AMNTDDA</b> (Mạng Đồ thị Chú ý Đa phương thức) đã phát hiện mẫu liên kết tiềm năng giữa <b style="color:#fff">${dInfo.name||'dược chất này'}</b> và bệnh <b style="color:#a5b4fc">${dName}</b> dựa trên đặc trưng Topology (cấu trúc liên kết đồ thị) và Embedding phân tử.</p>
                    </div>
                </div>
                <div class="hcard">
                    <div style="font-size:0.65rem;color:#00ffcc;font-weight:800;letter-spacing:1px;margin-bottom:1rem">🧬 CẤU TRÚC HÓA HỌC & ĐẶC TRƯNG</div>
                    <div style="display:flex; justify-content:center; align-items:center; background:var(--bg-secondary); border-radius:12px; padding:10px; margin-bottom:1rem; height: 180px; overflow: hidden;">
                        <img src="${imgUrl}" style="max-width:100%; max-height:100%; object-fit:contain; filter: contrast(1.2);" onerror="if(!this.dataset.fallback){ this.dataset.fallback='true'; this.src='${fallbackImgUrl}'; } else { this.onerror=null; this.parentElement.innerHTML='<div style=\\'color:#94a3b8; font-size:0.8rem; text-align:center;\\'><i class=\\'fas fa-image-slash\\' style=\\'font-size:2rem; margin-bottom:0.5rem;\\'></i><br>Không tìm thấy ảnh cấu trúc phân tử</div>'; }">
                    </div>
                    <canvas id="hubRadar" height="150"></canvas>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;margin-bottom:1.5rem;">
                <div class="hcard">
                    <div style="font-size:0.65rem;color:#10b981;font-weight:800;letter-spacing:1px;margin-bottom:1rem">💊 THÔNG TIN LÂM SÀNG</div>
                ...
                <div class="hcard">
                    <div style="font-size:0.65rem;color:#6366f1;font-weight:800;letter-spacing:1px;margin-bottom:1rem">🔬 CÁC DƯỢC CHẤT TƯƠNG TỰ</div>
                    ${simHtml}
                </div>
            </div>
            ...
            <div class="hcard" style="text-align:center;padding:2rem;">
                <i class="fas fa-book-medical" style="font-size:2rem;color:#38bdf8;margin-bottom:1rem"></i>
                <h4 style="color:#fff;margin:0 0 .5rem;font-size:1rem">Tìm kiếm Tài liệu Khoa học</h4>
                <p style="color:#64748b;font-size:0.8rem;margin-bottom:1.5rem">Tìm bài báo nghiên cứu về <b style="color:#a5b4fc">${dInfo.name||'dược chất'}</b> và <b style="color:#a5b4fc">${dName}</b> trên các cơ sở dữ liệu khoa học.</p>
        ...
        setTimeout(() => {
            ...
            if (ctx && typeof Chart !== 'undefined') {
                new Chart(ctx, {
                    ...
                    options: { scales: { r: { grid: { color: 'rgba(255,255,255,0.06)' }, angleLines: { color: 'rgba(255,255,255,0.06)' }, pointLabels: { color: '#64748b', font: { size: 10 } }, ticks: { display: false }, min: 0, max: 100 } }, plugins: { legend: { display: false } } }
                });
            }
        }, 100);
        ...
        fetch(`api/proxy.php?action=pathway&drug_idx=${pDrugIdx}&disease_idx=${pDiseaseIdx}&dataset=${document.getElementById('global-dataset')?.value || 'C-dataset'}`)
            ...
            .then(data => {
                ...
                html += '<div style="font-size:0.65rem;color:#ec4899;font-weight:800;letter-spacing:1px;margin-bottom:1.5rem"><i class="fas fa-project-diagram"></i> GNN-PREDICTED BIOLOGICAL PATHWAY</div>';
    ...
    function renderLandscape(predictions) {
        ...
        // Map prediction nodes to real coordinates using disease_idx
        const predictionNodes = predictions.map(p => {
            const diseaseIdx = p.disease_idx;
            if (normCoords.length > 0 && diseaseIdx < normCoords.length) {
                const coord = normCoords[diseaseIdx];
                return { x: coord.x, y: coord.y, label: p.disease_name, score: p.score };
            }
            // Fallback: use first few background coords sequentially
            const fallbackIdx = predictions.indexOf(p) % Math.min(backgroundData.length, 50);
            return { x: backgroundData[fallbackIdx].x, y: backgroundData[fallbackIdx].y, label: p.disease_name, score: p.score };
        });
                    ...
                    tooltip: {
                        callbacks: {
                            label: function(context) {
                                const p = predictionNodes[context.dataIndex];
                                return p ? `${p.label} (Score: ${p.score}%)` : '';
                            }
                        }
                    }
                },
